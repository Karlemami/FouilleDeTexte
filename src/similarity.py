...
def get_cosine_similarity(df1: pd.DataFrame, df2: pd.DataFrame) -> float:
    ...
    vectorizer.fit(
        [s1, s2]
    )  # Le vectorizer doit avoir le même vocabulaire pour les deux corpus, sinon on peut pas comparer.
    m1 = vectorizer.transform([s1])
    m2 = vectorizer.transform([s2])
    similarity = cosine_similarity(m1, m2)[0][
        0
    ]  # renvoie une np.array similarité document par document. On a que deux documents : les deux corpus qu'on a converti en string
    return similarity
...
def main():
    ...
    for genre1 in genre_matrices:
        for genre2 in genre_matrices:  # On fait les calculs pour chaque paire de genre
            if genre1 != genre2:
                size_genre1 = len(
                    genre_matrices[genre1]
                )  # On veut que les deux corpus aient la même taille.
                ...
                classification = classification_report(
                    predictions, classes, output_dict=True
                )
    ...
    for pair in similarity_and_report:
        print(
            f"{pair[0]}--{pair[1]}: similarity : {similarity_and_report[pair][0]}     accuracy : {similarity_and_report[pair][1]} "
        )
